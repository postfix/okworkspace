...
- **Provider: DeepSeek** (user decision — "use DeepSeek, key already in env"). **Model: `deepseek-v4-flash`** via the OpenAI-compatible endpoint — `base_url: https://api.deepseek.com/v1`, `api_key_env: DEEPSEEK_API_KEY` (key already present in the environment, 35 chars), `enabled: true`. Provider-agnostic per locked decision — no code change vs Ollama/OpenAI, only `config.yaml` + `eino-ext openai.ChatModelConfig{BaseURL, APIKey, Model}` values; the model is a config value, freely overridable. **Decision update (2026-06-21, from RESEARCH §Item 5):** the model was changed from `deepseek-chat` to `deepseek-v4-flash` because `deepseek-chat`/`deepseek-reasoner` **retire 2026-07-24** (~5 weeks out) and alias to the non-thinking `deepseek-v4-flash` successor, which supports the function/tool-calling the Eino ReAct loop requires. Shipping a model id that retires mid-milestone is an explicit anti-pattern (RESEARCH Pitfall 1). The provider choice (DeepSeek) is unchanged; only the specific model id — originally an implementation fill-in, not an explicit user pick — was corrected for durability. Anyone wanting the legacy alias for the next ~5 weeks can set `agent.model: deepseek-chat` in `config.yaml`.
