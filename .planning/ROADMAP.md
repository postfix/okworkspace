...
**Plans**: 6 plans
- [ ] 04-01-PLAN.md — Slice 1: pin eino v0.9.9 + eino-ext openai v0.1.13 + go-udiff; DeepSeek config (deepseek-v4-flash); agent.Service + ChatModel + key-gated smoke test; 3 audit constants
- [ ] 04-02-PLAN.md — Slice 2: 5 read-only tools + D5 allow-list build gate + ReAct Ask-page over SSE (AGNT-01/11)
- [ ] 04-03-PLAN.md — Slice 3: Ask scope expansion — selection/attachment/workspace-RAG with role-scoped retrieval + citations (AGNT-02/03/04)
- [ ] 04-04-PLAN.md — Slice 4: single-shot Summarize/Rewrite/Draft + validateProposedBody+retry (AGNT-05/06/07/08)
- [ ] 04-05-PLAN.md — Slice 5 (safety core): propose→server-diff→approve→apply; D4 round-trip + D8 stale-409 tests (AGNT-09/10/11)
