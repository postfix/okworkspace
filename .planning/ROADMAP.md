...
- [ ] 10-01-PLAN.md — testable core: install `react-force-graph-2d` (Canvas-only, lockfile `three`-absence guard) + `getGraph`/`getLocalGraph` api fns & hooks + `graphEdges` zustand toggle slice + pure degree/orphan/filter/highlight helpers + thin `GraphCanvas` ref wrapper + `--graph-node-orphan` token (GRAPH-01..05)
